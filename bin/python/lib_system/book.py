...
class Book (Document) :
	...
	def addComponent (self, aComp) :
		'''This adds a componet to a list of components in an object in
		memory.'''
		self._components.append(aComp)
		return aComp


	def addToBinding (self, compID) :
		'''This will do a simple component append to the bindingOrder list and
		add it to the project.conf file.  This will not look at the other
		components but just blindly append the compID.  More than one instance
		can be in the list. [Note: more complex functions will need to be
		...
		# bindingOrder list.
		self._config['bindingOrder'] = self._config['bindingOrder'] + [compID]


	def removeFromBinding (self, compID) :
		'''Remove a component from the binding list.  This will remove all
		instances of the compID.'''

# FIXME: Adding and removing ids from the comp object and binding list doesn't
# work because the depend on each other in a kind of circular way.

		cList = self._config['bindingOrder']
		cList.remove(compID)
		self._config['bindingOrder'] = cList
		for i in cList :
			print i
